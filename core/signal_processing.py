...
class SignalProcessor:
    ...
    def design_filter(
        ...
        **kwargs: Any,
        ...
        if normalized_cutoff >= 1.0:
            warnings.warn(
                f"Cutoff frequency {cutoff_freq} Hz exceeds Nyquist frequency"
            )
        ...
        if filter_type.lower() == "butterworth":
            ...
            sos = signal.cheby1(
                order, ripple, normalized_cutoff, btype="low", output="sos"
            )
        elif filter_type.lower() == "elliptic":
            ripple = kwargs.get("ripple", 0.5)
            attenuation = kwargs.get("attenuation", 40)
            sos = signal.ellip(
                order, ripple, attenuation, normalized_cutoff, btype="low", output="sos"
            )
        else:
    ...
    def estimate_carrier_frequency(
        self, iq_samples: IQSamples, sample_rate: float, method: str = "fft_peak"
    ) -> float:
        """
        ...
        if method == "fft_peak":
            ...
            peaks, _ = find_peaks(
                np.abs(autocorr[1:]), height=np.max(np.abs(autocorr)) * 0.1
            )
            if len(peaks) > 0:
    ...
    def estimate_snr(
        self, signal_data: Union[IQSamples, RealSamples], method: str = "moment"
    ) -> float:
        """
        ...
        if method == "moment":
            ...
            if len(signal_data) >= 256:
                freqs, psd = signal.welch(
                    signal_data, nperseg=min(256, len(signal_data) // 4)
                )
    ...
    def frequency_shift(
        self, iq_data: IQSamples, shift_hz: float, sample_rate: float
    ) -> IQSamples:
            ...
            metrics.update(
                {
                    ...
                    "avg_snr_db": float(np.mean(self.snr_history))
                    if self.snr_history
                    else 0.0,
                    "sample_loss_rate": (self.dropped_samples / self.total_samples)
                    * 100.0,
                }
            )
    ...
    def detect_anomalies(self, current_metrics: QualityMetrics) -> List[str]:
        ...
        if power_history:
            ...
            if abs(current_power - avg_power) > 3 * power_std:
                anomalies.append(
                    f"Power anomaly: {current_power:.1f} dBFS (avg: {avg_power:.1f})"
                )
...
def iq_to_complex(
    i_samples: Union[List, npt.NDArray], q_samples: Union[List, npt.NDArray]
) -> npt.NDArray:
    """
...
def frequency_shift(
    iq_data: npt.NDArray, shift_hz: float, sample_rate: float
) -> npt.NDArray:
...
def analyze_signal_quality(
    iq_data: Union[IQSamples, RealSamples], sample_rate: Optional[float] = None
) -> QualityMetrics:
    """
    ...
    if sample_rate is not None:
        try:
            freqs, psd = signal.welch(
                iq_data, fs=sample_rate, nperseg=min(1024, len(iq_data) // 4)
            )
...
def correct_iq_imbalance(iq_data: IQSamples) -> IQSamples:
    ...
    if abs(cross_correlation) > 0.1:
        ...
        q_corrected = q_corrected * np.cos(phase_correction) - i_samples * np.sin(
            phase_correction
        )
...
def estimate_modulation_scheme(
    iq_data: IQSamples, sample_rate: float
) -> Dict[str, Any]:
    """
...
def create_test_signal(
    signal_type: str, sample_rate: float, duration: float, **parameters: Any
) -> IQSamples:
    """
    ...
    if signal_type == "sine":
        ...
        for i, symbol in enumerate(symbols):
            ...
            signal_data[start_idx:end_idx] = np.exp(
                1j * (2 * np.pi * carrier_freq * t_symbol + phase)
            )
...
def convert_power_units(
    value: float, from_unit: str, to_unit: str, reference_impedance: float = 50.0
) -> float:
...
if __name__ == "__main__":
    ...
    print(
        f"Normalized signal power: {processor.calculate_power_dbfs(normalized):.1f} dBFS"
    )
