...
try:
    from .demodulation import ModulationScheme
    from .parsing import DroneProtocol
...
# Minimal stub so type annotations resolve when demodulation module is absent
try:
    from .demodulation import DemodulationResult
except ImportError:

    class DemodulationResult:  # type: ignore[no-redef]
        pass


try:
    from .parsing import PacketResult
except ImportError:

    class PacketResult:  # type: ignore[no-redef]
        pass
...
class ClassificationResult:
    ...
    top_k_predictions: List[Tuple[ProtocolLabel, ConfidenceScore]] = field(
...
class AdvancedFeatureExtractor:
    ...
    def extract_features(
        ...
        for feature_type in self.feature_types:
            if feature_type == FeatureType.STATISTICAL:
                ...
                feats, names = self._extract_protocol_features(
                    packet_data, packet_result
                )
    ...
    def _extract_statistical_features(
        self, data: npt.NDArray[np.uint8]
    ...
    def _extract_spectral_features(
        ...
        features = [
            ...
            float(
                np.sqrt(
                    np.sum((freqs - np.sum(freqs * psd) / np.sum(psd)) ** 2 * psd)
                    / np.sum(psd)
                )
            ),  # Spectral spread
            float(np.sum(psd[: len(psd) // 4]) / np.sum(psd)),  # Low frequency ratio
            float(
                np.sum(psd[3 * len(psd) // 4 :]) / np.sum(psd)
    ...
    def _extract_temporal_features(
        ...
        if len(data) > 8:
            ...
            if len(autocorr_norm) > 2:
                first_min_idx = (
                    np.argmin(autocorr_norm[1 : len(autocorr_norm) // 4]) + 1
                )
    ...
    def _extract_protocol_features(
        self, packet_data: bytes, packet_result: Optional[PacketResult] = None
    ) -> Tuple[List[float], List[str]]:
        ...
        features.extend(
            [
                ...
                float(packet_data[0])
                if packet_data
                else 0,  # First byte (potential sync/magic)
        ...
        if len(packet_data) >= 2:
            ...
            features.extend(
                [float(is_mavlink_v1), float(is_mavlink_v2), float(is_dji_sync)]
            )
        ...
        if len(packet_data) >= 8:
            ...
            if packet_data[0] in [0xFE, 0xFD]:
                ...
                expected_total = (
                    (8 + payload_len) if packet_data[0] == 0xFE else (12 + payload_len)
                )
        ...
        if packet_result is not None:
            ...
            names.extend(
                [
                    "proto_parser_confidence",
                    "proto_header_valid",
                    "proto_checksum_valid",
                    "proto_signal_power",
                    "proto_snr",
                ]
            )
    ...
    def _extract_signal_features(
        self, signal_metrics: Optional[Dict[str, float]]
    ) -> Tuple[List[float], List[str]]:
    ...
    def _extract_modulation_features(
        self, demod_result: Optional[DemodulationResult]
    ) -> Tuple[List[float], List[str]]:
        ...
        if (
            hasattr(demod_result, "evm_percent")
            and demod_result.evm_percent is not None
        ):
...
class ModelManager:
    ...
    def load_models(self) -> bool:
        ...
        try:
            ...
            if primary_model_file.exists():
                self.models[self.config.primary_method.value] = joblib.load(
                    primary_model_file
                )
    ...
    def create_fallback_model(self) -> bool:
        """Intentionally does not create a fallback model.

        A classifier trained on random noise produces meaningless predictions
        and cannot be trusted in research or operational contexts.  Callers
        must train a real model via ``train()`` or load one from disk.
        """
        logger.error(
            "No trained classification models are available.  "
            "Run 'dronecmd train' to build models from labelled IQ captures, "
            "or point --model-path at a directory containing trained .pkl files."
        )
        return False
...
class EnhancedProtocolClassifier:
    ...
    def __init__(
        ...
        # Load models — explicit failure if none found
        models_loaded = self.model_manager.load_models()
        if not models_loaded:
            self.model_manager.create_fallback_model()  # logs the actionable error

        self._models_ready = models_loaded
        logger.info(
            f"Initialized protocol classifier: "
            f"{len(self.model_manager.models)} model(s) loaded, "
            f"ready={self._models_ready}"
        )
    ...
    def classify(
        ...
        if not self._models_ready:
            raise ModelNotTrainedError(
                "No trained models are loaded.  "
                "Run 'dronecmd train' or load models via ClassifierConfig.model_path."
            )
        ...
        try:
            ...
            if self.config.enable_ensemble and len(self.model_manager.models) > 1:
                ...
                classification_result = self._single_model_classify(
    ...
    def _single_model_classify(
        self, features: FeatureVector, feature_names: List[str]
    ...
    def _update_statistics(
        self, result: ClassificationResult, processing_time: float
    ) -> None:
    ...
    def get_performance_stats(self) -> Dict[str, Any]:
        ...
        avg_time = self.stats["total_processing_time"] / max(
            1, self.stats["classifications_performed"]
        )
...
def create_protocol_classifier(
