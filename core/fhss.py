...
class FHSSBand(Enum):
    ...
    def min_channels(self) -> int:
        ...
        minimums = {self.ISM_900_MHz: 25, self.ISM_2_4_GHz: 75, self.ISM_5_8_GHz: 75}
...
class PulseShapeFilter:
    ...
    def raised_cosine_filter(
        cls, beta: float, span: int, sps: int
...
class FHSSCore:
    ...
    def _generate_channel_frequencies(self) -> None:
        ...
        self.channel_frequencies = np.array(
            [
                ...
                + (i - half_span) * self.config.channel_spacing_hz
    ...
    def generate_hop_sequence(
        self, length: int, seed: Optional[int] = None
    ) -> List[HopIndex]:
...
class SimpleFHSS:
    ...
    def prepare_transmit_frames(
        ...
        frames = enhanced.prepare_transmit_frames(
            packet, sample_rate, bitrate, hop_duration_ms, None, rc_beta, rc_span, seed
        )
...
class EnhancedFHSSEngine:
    ...
    def prepare_transmit_frames(
        ...
        for hop_idx, chunk in zip(hop_indices, chunks):
            ...
            duration_s = (
                hop_duration_ms / 1000.0
                if hop_duration_ms is not None
    ...
    def get_channel_utilization_stats(
        self, hop_sequence: List[HopIndex]
    ) -> Dict[str, Any]:
...
def create_fhss_engine(
    ...
    **kwargs: Any,
    ...
    if simple:
        ...
        simple_kwargs = {
            k: v for k, v in kwargs.items() if k in ["seed", "hop_sequence"]
        }
...
def create_fcc_compliant_fhss(
    band: FHSSBand, center_freq_hz: Optional[FrequencyHz] = None, **kwargs: Any
) -> EnhancedFHSSEngine:
    """
...
if __name__ == "__main__":
    ...
    enhanced_fhss = create_fcc_compliant_fhss(
        FHSSBand.ISM_2_4_GHz, channel_spacing_hz=1e6
    )
