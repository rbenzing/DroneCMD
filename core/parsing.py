...
# Integration with enhanced modules — note: no import from classification here
# to avoid the circular dependency (classification imports parsing).
try:
    ...
    # Provide a minimal ModulationScheme so default argument annotations resolve
    class ModulationScheme(Enum):  # type: ignore[no-redef]
        OOK = "ook"
        FSK = "fsk"
        GFSK = "gfsk"
        BPSK = "bpsk"
...
class PacketResult:
    ...
    def is_valid(self) -> bool:
        ...
        return (
            self.protocol != DroneProtocol.UNKNOWN
...
class MAVLinkParser(ProtocolParserBase):
    ...
    def _parse_mavlink_v1(self, packet_bytes: PacketBytes) -> PacketResult:
        ...
        if len(packet_bytes) < expected_total_len:
            return PacketResult(
                ...
                packet_bytes=packet_bytes,
                parse_errors=[
                    f"Incomplete packet: {len(packet_bytes)} < {expected_total_len}"
                ],
            )
    ...
    def _parse_mavlink_v2(self, packet_bytes: PacketBytes) -> PacketResult:
        ...
        msg_id = int.from_bytes(
            packet_bytes[7:10], byteorder="little"
        )  # Message ID (24-bit)
...
class EnhancedPacketParser:
    ...
    def __init__(
        ...
        logger.info(
            f"Initialized enhanced packet parser with {len(self.parsers)} protocol parsers"
        )
            ...
            if self.classifier is not None:
                try:
                    ...
                    if hasattr(classification_result, "predicted_protocol"):
                        ...
                        logger.debug(
                            f"Classifier detected: {protocol_name} ({confidence:.3f})"
                        )
    ...
    def parse_iq_samples(
        ...
        try:
            ...
            if self.signal_processor:
                packet_regions = detect_packets(
                    iq_samples, threshold=0.05, min_gap=1000
                )
            else:
            ...
            logger.info(
                f"Parsed {len(results)} packets from {len(packet_regions)} detected regions"
            )
            return results
    ...
    def get_statistics(self) -> Dict[str, Any]:
        ...
        if stats["packets_parsed"] > 0:
            stats["average_processing_time_ms"] = (
                stats["total_processing_time"] / stats["packets_parsed"]
            ) * 1000
        else:
...
class PacketParser:
    ...
    def __init__(
        self, preamble_patterns: Optional[Dict[str, npt.NDArray]] = None
    ) -> None:
        """
    ...
    def parse(self, iq_samples: npt.NDArray, sample_rate: int) -> List[Dict[str, Any]]:
        ...
        try:
            # Use enhanced parser
            results = self._enhanced_parser.parse_iq_samples(iq_samples, sample_rate)
    ...
    def extract_packets(
        self, iq_data: npt.NDArray, start_indices: List[int], length: int
    ) -> List[npt.NDArray]:
