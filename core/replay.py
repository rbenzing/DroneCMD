...
try:
    from .fhss import EnhancedFHSSEngine, FHSSConfig, HopFrame
    from .demodulation import DemodulationEngine, DemodConfig, ModulationScheme
    from .parsing import EnhancedPacketParser
    from .classification import EnhancedProtocolClassifier, ClassifierConfig
    from .capture import EnhancedLiveCapture, SDRConfig
...
try:
    ...
    try:
        ...
        class FrequencyViolationError(Exception):
            pass  # type: ignore[no-redef]

        class DwellTimeError(Exception):
            pass  # type: ignore[no-redef]
...
class ComplianceMonitor:
    ...
    def check_timing_compliance(self, duration_s: float) -> None:
        """Assert transmission duration is within safety timeout.

        Raises:
            DwellTimeError: if duration_s exceeds safety_timeout_s.
        """
        if duration_s > self.config.safety_timeout_s:
            msg = (
                f"Continuous transmission {duration_s:.1f}s exceeds "
                f"safety timeout {self.config.safety_timeout_s:.1f}s"
            )
    ...
    def get_compliance_report(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "power_stats": {
                "mean_dbm": float(np.mean(self.power_measurements))
                if self.power_measurements
                else 0.0,
                "max_dbm": float(np.max(self.power_measurements))
                if self.power_measurements
                else 0.0,
                "samples": len(self.power_measurements),
            },
            "frequency_stats": {
                "mean_hz": float(np.mean(self.frequency_measurements))
                if self.frequency_measurements
                else 0.0,
                "range_hz": float(np.ptp(self.frequency_measurements))
                if self.frequency_measurements
                else 0.0,
...
class ReplayStrategyBase(ABC):
    ...
    async def execute_replay(
        self, transmitter: Any, packet_data: bytes, repeat_count: int, **kwargs: Any
    ) -> ReplayResult:
        ...
        **kwargs: Any,
        ...
        for i in range(repeat_count):
            try:
                ...
                # Check compliance before transmission — raises DwellTimeError on violation
                try:
                    self.compliance_monitor.check_timing_compliance(
                        self.compliance_monitor.get_transmission_duration()
                    )
                except DwellTimeError as e:
        ...
        result.packets_transmitted = (
            result.successful_transmissions + result.failed_transmissions
        )
...
class IntelligentReplayStrategy(ReplayStrategyBase):
        ...
        for i in range(repeat_count):
            try:
                ...
                if protocol_info and protocol_info.get("allow_modification", False):
                    modified_packet = self._modify_packet_intelligently(
                        packet_data, protocol_info
                    )

                # Transmit with quality monitoring
                transmission_start = time.perf_counter()
                await self._transmit_with_monitoring(
                    transmitter, modified_packet, result
                )
                transmission_time = time.perf_counter() - transmission_start
                ...
                if adaptive_timing and i % 10 == 9:  # Every 10 packets
                    optimal_timing = self._adapt_timing_parameters(
                ...
                result.error_messages.append(
                    f"Intelligent transmission {i+1} failed: {str(e)}"
                )
        ...
        try:
            ...
            if hasattr(classification_result, "predicted_protocol"):
                return {
                    ...
                    if "mavlink" in classification_result.predicted_protocol.lower()
                    else "medium",
                }
            else:
                return {
    ...
    def _calculate_optimal_timing(
        self, protocol_info: Optional[Dict[str, Any]], adaptive: bool
    ) -> Dict[str, float]:
        ...
        return {
            ...
            "timing_tolerance": base_delay * 0.1
            if timing_sensitivity == "high"
            else base_delay * 0.3,
        }

    def _modify_packet_intelligently(
        self, packet_data: bytes, protocol_info: Dict[str, Any]
    ) -> bytes:
    ...
    async def _transmit_with_monitoring(
        self, transmitter: Any, packet_data: bytes, result: ReplayResult
    ) -> None:
        ...
        if not self.compliance_monitor.check_power_compliance(simulated_power):
            result.warnings.append(
                f"Power compliance violation: {simulated_power:.1f} dBm"
            )
    ...
    def _adapt_timing_parameters(
        self, current_timing: Dict[str, float], result: ReplayResult
    ) -> Dict[str, float]:
...
class EnhancedReplayEngine:
    ...
    def __init__(
        ...
        logger.info(
            f"Initialized enhanced replay engine with {config.strategy.value} strategy"
        )
    ...
    def _create_strategy(self) -> ReplayStrategyBase:
        ...
        if self.config.strategy == ReplayStrategy.SIMPLE:
            ...
            logger.warning(
                f"Strategy {self.config.strategy.value} not implemented, using simple"
            )
            return SimpleReplayStrategy(self.config)
    ...
    async def stress_test_replay(
        self, packet_bytes: bytes, target_rate_hz: float, duration_s: float
    ) -> ReplayResult:
        """
        ...
        if target_rate_hz > self.config.max_packet_rate_hz:
            raise ValueError(
                f"Target rate {target_rate_hz} Hz exceeds maximum {self.config.max_packet_rate_hz} Hz"
            )
        ...
        logger.info(
            f"Starting stress test: {total_packets} packets at {target_rate_hz} Hz for {duration_s}s"
        )
        ...
        try:
            result = await self.strategy.execute_replay(
                self.transmitter, packet_bytes, total_packets
            )
            ...
            if result.inter_packet_intervals_s:
                ...
                if rate_error > 0.1:  # 10% tolerance
                    result.warnings.append(
                        f"Rate error {rate_error*100:.1f}%: target {target_rate_hz} Hz, actual {actual_rate:.1f} Hz"
                    )
    ...
    async def _apply_fhss_if_enabled(self, packet_bytes: bytes) -> bytes:
        ...
        Splits the packet across hop frames and reassembles the payload chunks.
        The transmitter is responsible for applying per-frame frequency and
        IQ modulation; we return the ordered chunk bytes for the strategy layer.
        """
        if not self.config.enable_fhss or not self.fhss_engine:
            return packet_bytes

        try:
            frames: List[HopFrame] = self.fhss_engine.prepare_transmit_frames(
                packet_bytes
            )
            if not frames:
                logger.warning("FHSS engine returned no frames; using original packet")
                return packet_bytes
            # Reassemble payload in hop order so the strategy transmits the
            # correctly interleaved bytes.  Frequency/timing metadata lives in
            # the HopFrame objects and is consumed by the transmitter interface.
            return b"".join(frame.chunk_data for frame in frames)
        except Exception as e:
            logger.warning(f"FHSS processing failed, using original packet: {e}")
            return packet_bytes
    ...
    async def _validate_packet(self, packet_bytes: bytes) -> Dict[str, Any]:
        ...
        if len(packet_bytes) > 2048:  # Reasonable maximum
            validation_result["warnings"].append(
                f"Large packet: {len(packet_bytes)} bytes"
            )
    ...
    def get_performance_stats(self) -> Dict[str, Any]:
        ...
        avg_transmission_time = self.stats["total_transmission_time"] / max(
            1, self.stats["total_replays"]
        )

        avg_packets_per_replay = self.stats["total_packets_transmitted"] / max(
            1, self.stats["total_replays"]
        )
...
async def main() -> None:
    ...
    print(
        f"  Simple: {simple_config.strategy.value}, precision: {simple_config.timing_precision_us}μs"
    )
    print(
        f"  Intelligent: {intelligent_config.strategy.value}, protocol-aware: {intelligent_config.enable_protocol_awareness}"
    )
    ...
    simple_result = await simple_engine.replay_packet(
        test_packets[0], repeat_count=3, random_delay=True
    )
    ...
    intelligent_result = await intelligent_engine.replay_packet(
        test_packets[0], repeat_count=5, adaptive_timing=True
    )
    ...
    print(
        f"  Average success rate: {np.mean([r.success_rate for r in sequence_results]):.1%}"
    )
