...
try:
    from .parsing import EnhancedPacketParser, ParserConfig, PacketResult, DroneProtocol
...
def create_capture_system(
    platform: str = "rtl_sdr", **config_kwargs: Any
) -> Optional[Any]:
    """
...
def create_demodulation_system(
    scheme: Union[str, ModulationScheme], **config_kwargs: Any
) -> Optional[Any]:
    """
...
def validate_core_integration() -> Dict[str, Any]:
    ...
    """
    results = {"status": "ok", "issues": [], "warnings": [], "recommendations": []}
    ...
    if CAPTURE_AVAILABLE and SIGNAL_PROCESSING_AVAILABLE:
        results["recommendations"].append(
            "Capture + Signal Processing: Full pipeline available"
        )

    if DEMODULATION_AVAILABLE and CLASSIFICATION_AVAILABLE:
        results["recommendations"].append(
            "Demod + Classification: Complete protocol analysis available"
        )

    if FHSS_AVAILABLE and REPLAY_AVAILABLE:
        results["recommendations"].append(
            "FHSS + Replay: Advanced transmission testing available"
        )
...
missing_systems = [
    name for name, status in get_core_system_status().items() if not status
]
if missing_systems:
    warnings.warn(
