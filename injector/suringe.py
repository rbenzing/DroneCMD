...
class InjectionConfig:
    ...
    def __post_init__(self) -> None:
        pass  # Validation is explicit via validate() — callers check errors list
    ...
    def validate(self) -> List[str]:
        ...
        if self.inter_packet_delay_s < 0:
            errors.append("inter_packet_delay_s must be non-negative")
        ...
        if self.max_transmission_power_dbm > 30.0:
            errors.append(
                f"max_transmission_power_dbm {self.max_transmission_power_dbm} dBm "
                "exceeds 30 dBm hard limit (FCC CFR 47 §15.247)"
            )
        ...
        if self.jitter_range_s[0] >= self.jitter_range_s[1]:
            errors.append("jitter_range_s lower bound must be less than upper bound")
        return errors
...
class SafetyMonitor:
    ...
    def check_power_compliance(self, power_dbm: float) -> None:
        """Assert transmission power is within configured limits.

        Raises:
            PowerLimitError: if power_dbm exceeds max_transmission_power_dbm.
        """
        self.power_measurements.append(power_dbm)
        if power_dbm > self.config.max_transmission_power_dbm:
            msg = (
                f"Power {power_dbm:.1f} dBm exceeds hard limit "
                f"{self.config.max_transmission_power_dbm:.1f} dBm"
            )
            self.violations.append(msg)
            raise PowerLimitError(msg)

    def check_duration_compliance(self) -> None:
        """Assert injection session is within configured time limit.

        Raises:
            DwellTimeError: if the session has run longer than max_injection_duration_s.
        """
        ...
        if duration > self.config.max_injection_duration_s:
            msg = (
                f"Injection duration {duration:.1f}s exceeds limit "
                f"{self.config.max_injection_duration_s:.1f}s"
            )
            self.violations.append(msg)
...
class InjectionEngine:
    ...
    def __init__(
        ...
        logger.info(
            f"Initialized injection engine with {self.config.method.value} method"
        )
    ...
    def _init_enhanced_components(self) -> None:
        ...
        try:
            ...
            if self.config.replay_config:
                ...
                self._replay_engine = EnhancedReplayEngine(
                    replay_config, self.transmitter
                )
    ...
    def inject_packet(
        ...
        actual_jitter = (
            ...
            else self.config.enable_timing_jitter
        )
        ...
        if power_dbm is not None:
            # Enforce power limit before touching hardware
            self.safety_monitor.check_power_compliance(power_dbm)
        ...
        try:
            for i in range(actual_repeat):
                # Check safety compliance — raises DwellTimeError on violation
                try:
                    self.safety_monitor.check_duration_compliance()
                except DwellTimeError as e:
                    result.safety_violations.append(str(e))
                    break
            ...
            result.packets_injected = (
                result.successful_injections + result.failed_injections
            )
            ...
            for i in range(repeat):
                ...
                try:
                    ...
                    result.error_messages.append(
                        f"Async injection {i+1} failed: {str(e)}"
                    )
        ...
        try:
            ...
            for frame in frames:
                ...
                try:
                    ...
                    # Apply obfuscation if enabled
                    processed_data = self._apply_obfuscation(
                        frame_data, frame.hop_index
                    )
                    ...
                    result.failed_injections += 1
                    result.error_messages.append(
                        f"FHSS frame {frame.hop_index} failed: {str(e)}"
                    )
    ...
    async def inject_with_replay(
        self, recorded_signal: Union[bytes, IQSamples], repeat: int = 1, **kwargs: Any
    ) -> InjectionResult:
        """
        ...
        try:
            ...
            replay_result = await self._replay_engine.replay_packet(
                packet_data, repeat_count=repeat, **kwargs
            )
    ...
    def inject_continuous(
        ...
        **kwargs: Any,
        ...
        if duration_s > self.config.max_injection_duration_s:
            raise ValueError(
                f"Duration {duration_s}s exceeds maximum {self.config.max_injection_duration_s}s"
            )
        ...
        try:
            ...
            while time.time() - start_time < duration_s:
                ...
                try:
                    ...
                    result.failed_injections += 1
                    result.error_messages.append(
                        f"Continuous injection {packet_count} failed: {str(e)}"
                    )
            ...
            logger.info(
                f"Continuous injection: {packet_count} packets in {result.total_injection_time_s:.3f}s"
            )
            return result
...
class GhostInjector:
    ...
    def craft_and_send(
        self, command: Dict[str, Any], repeat: int = 3, jitter: bool = True
    ) -> None:
        """
        ...
        try:
            ...
            result = self.injection_engine.inject_packet(
                packet, repeat=repeat, enable_jitter=jitter
            )

            logger.info(
                f"Ghost injection completed: {result.successful_injections}/{repeat} packets"
            )
...
def create_injection_engine(
    transmitter: TransmitterInterface, method: str = "simple", **kwargs: Any
) -> InjectionEngine:
    """
...
if __name__ == "__main__":
    ...
    try:
        ...
        stealth_injector = create_injection_engine(
            transmitter, method="stealth", enable_timing_jitter=True
        )
        ...
        if ENHANCED_AVAILABLE:
            ...
            try:
                ...
                fhss_result = fhss_injector.inject_with_fhss(
                    test_packet, band=FHSSBand.ISM_2_4_GHz
                )
                print(f"FHSS injection result:")
                print(f"  Packets injected: {fhss_result.packets_injected}")
                print(
                    f"  Frequency accuracy: {fhss_result.frequency_accuracy_hz:.1f} Hz"
                )
            except Exception as e:
