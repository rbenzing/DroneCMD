...
class ComplianceChecker:
    ...
    def check_frequency_compliance(self, frequency_hz: float) -> bool:
        ...
        ism_433 = (
            self.frequency_limits["min_hz"]
            <= frequency_hz
            <= self.frequency_limits["max_hz"]
        )

        ism_2_4 = (
            self.frequency_limits["ism_2_4_min"]
            <= frequency_hz
        ...
        if not (ism_433 or ism_2_4):
            violation = (
                f"Frequency {frequency_hz/1e6:.1f} MHz outside allowed ISM bands"
            )
    ...
    def validate_injection_request(
        self, frequency_hz: float, power_dbm: float, duration_s: float
    ) -> Dict[str, Any]:
        """
...
class SafeInjector:
    ...
    def validate_injection(
        self, frequency_hz: float, power_dbm: float = 0.0, duration_s: float = 1.0
    ) -> Dict[str, Any]:
        """
...
def create_safe_injector(
    platform: str = "hackrf", **config_kwargs: Any
...
def check_injection_legality(
    frequency_hz: float, power_dbm: float, location: Optional[str] = None
) -> Dict[str, Any]:
    """
...
def display_legal_warning() -> None:
    """Display legal warning (ASCII-safe for all terminals)."""
    warning_text = (
        "\n"
        "  *** LEGAL WARNING ***\n"
        "  This injection module is for AUTHORIZED TESTING ONLY.\n"
        "  Users are legally responsible for FCC/regulatory compliance.\n"
        "  UNAUTHORIZED TRANSMISSION IS ILLEGAL.\n"
    )
    try:
        print(warning_text)
    except UnicodeEncodeError:
        print("  *** LEGAL WARNING: Authorized use only. See documentation. ***")
    logger.warning("Injection module loaded — authorized laboratory use only")
