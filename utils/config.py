...
class SDRConfig:
    ...
    def validate(self) -> List[str]:
        ...
        if not (
            self.frequency_range[0] <= self.default_frequency <= self.frequency_range[1]
        ):
...
class InjectionConfig:
    ...
    def validate(self) -> List[str]:
        ...
        Returns a list of error strings.  Callers must check this list and
        refuse to proceed when it is non-empty — these are hard limits, not
        suggestions.
        """
        errors = []
        ...
        if self.jitter_range_s[0] >= self.jitter_range_s[1]:
            errors.append(
                "invalid jitter_range_s: lower bound must be less than upper bound"
            )
...
class ConfigManager:
    ...
    def __init__(
        ...
        logger.info(
            f"Initialized ConfigManager for {self.environment.value} environment"
        )
    ...
    def load_from_file(
        self, filename: Optional[Union[str, Path]] = None, format: str = "auto"
    ) -> bool:
        """
        ...
        for env_var, config_path in env_mappings.items():
            ...
            if value is not None:
                self._set_nested_value(
                    env_overrides, config_path, self._convert_env_value(value)
                )
...
IQ_CAPTURE_PATH = _current_config.paths.get_absolute_path(
    _current_config.paths.capture_path
)
...
if __name__ == "__main__":
    ...
    try:
        ...
        print(
            f"  Classification threshold: {analysis_config.classification_confidence_threshold}"
        )
