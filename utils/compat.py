...
class DeprecationHelper:
    ...
    def warn_deprecated_module(
        self, old_module: str, new_module: str, version: str = "2.0.0"
    ) -> None:
...
def _safe_import(module_name: str, fallback_name: Optional[str] = None) -> Any:
    ...
    try:
        ...
        if fallback_name:
            logger.debug(
                f"Failed to import {module_name}, trying fallback {fallback_name}"
            )
            try:
...
class FHSSEngine:
    ...
    def __init__(
        ...
        _deprecation_helper.warn_deprecated_function(
            "dronecmd.utils.signal_tools.FHSSEngine", "dronecmd.core.fhss.SimpleFHSS"
        )
    ...
    def _init_fallback(
        self, center_freq_hz, channel_spacing_hz, hops, seed, hop_sequence
    ):
        ...
        self.channels = np.array(
            [center_freq_hz + (i - half_span) * channel_spacing_hz for i in range(hops)]
        )
    ...
    def prepare_transmit_frames(
        self, packet: bytes, sample_rate: int = 2_000_000, bitrate: int = 1000, **kwargs
        ...
        if hasattr(self._engine, "prepare_transmit_frames"):
            return self._engine.prepare_transmit_frames(
                packet, sample_rate, bitrate, **kwargs
            )
        else:
...
class CaptureManager:
    ...
    def __init__(
        self, freq_hz: float = 0, sample_rate: float = 2.048e6, duration_sec: float = 0
    ):
        ...
        if ENHANCED_MODULES_AVAILABLE and _capture_manager:
            self._manager = _capture_manager.CaptureManager(sample_rate=sample_rate)
            if freq_hz > 0:
...
def check_compatibility() -> Dict[str, Any]:
    ...
    dependencies = {"numpy": False, "scipy": False, "sklearn": False, "pyrtlsdr": False}
...
def wrap_legacy_plugin(plugin_class):
    ...
    class WrappedPlugin(plugin_class):
        def __init__(self, *args, **kwargs):
            _deprecation_helper.warn_deprecated_function(
                f"{plugin_class.__name__}", "dronecmd.plugins.base.BasePlugin"
            )
...
def _initialize_compatibility():
    ...
    if not check_python_version("3.8"):
        warnings.warn("Python version < 3.8 may not be fully supported", RuntimeWarning)
...
if __name__ == "__main__":
    ...
    try:
        ...
        test_signal = np.random.complex64(
            np.random.randn(1000) + 1j * np.random.randn(1000)
        )
