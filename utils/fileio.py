...
def read_iq_file(
    ...
    try:
        if file_format == FileFormat.COMPLEX64:
            return _read_complex64(
                filepath, max_samples, offset_samples, progress_callback
            )
        elif file_format == FileFormat.COMPLEX128:
            return _read_complex128(
                filepath, max_samples, offset_samples, progress_callback
            )
        elif file_format == FileFormat.INT16_IQ:
            return _read_int16_iq(
                filepath, max_samples, offset_samples, progress_callback
            )
        elif file_format == FileFormat.FLOAT32_IQ:
            return _read_float32_iq(
                filepath, max_samples, offset_samples, progress_callback
            )
...
def _read_wav_file(
    filepath: Path, max_samples: Optional[int], offset_samples: int
) -> IQSamples:
...
def _write_wav_file(
    filepath: Path, iq_samples: IQSamples, metadata: Optional[Dict[str, Any]]
) -> None:
...
def _write_sigmf_data(
    ...
    sigmf_meta = {
        ...
        "captures": [
            {
                "core:sample_start": 0,
                "core:frequency": metadata.get("center_frequency", 0.0)
                if metadata
                else 0.0,
...
def list_iq_files(
    directory: FilePath, recursive: bool = False, include_compressed: bool = True
) -> List[FileInfo]:
    """
    ...
    for filepath in directory.glob(pattern):
        if filepath.is_file() and any(
            str(filepath).lower().endswith(ext) for ext in iq_extensions
        ):
...
def verify_file_integrity(
    filepath: FilePath, expected_checksum: Optional[str] = None
) -> bool:
    """
...
if __name__ == "__main__":
    ...
    test_samples = (np.random.randn(10000) + 1j * np.random.randn(10000)).astype(
        np.complex64
    )
    ...
    write_iq_file(
        test_file,
        test_samples,
        metadata={"sample_rate": 2.048e6, "center_frequency": 100.1e6},
    )
