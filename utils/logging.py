...
def configure_logging(
    ...
    **kwargs: Any,
    ...
    if config.enable_console:
        ...
        formatter_name = (
            "json" if config.enable_structured else config.format_type.value
        )
...
def _create_formatters(config: LoggingConfig) -> Dict[str, logging.Formatter]:
    ...
    formatters["json"] = JSONFormatter(
        include_context=True, include_performance=config.enable_performance
    )
        ...
        def wrapper(*args, **kwargs):
            ...
            with log_performance(func_name, logger):
                try:
                    ...
                    logger.log(
                        level, f"Exiting function: {func_name}", extra=exit_extra
                    )
...
def create_operation_logger(operation_name: str) -> logging.Logger:
    ...
    operation_id = str(uuid.uuid4())
    set_context({"operation": operation_name, "operation_id": operation_id})
...
if __name__ == "__main__":
    ...
    with log_context({"component": "demo", "version": "1.0"}):
        logger.info(
            "Structured message",
            extra={"user_id": 12345, "action": "login", "success": True},
        )
