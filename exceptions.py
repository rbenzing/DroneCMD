...
class DeviceNotFoundError(SDRError):
    ...
    def __init__(
        self, device_type: str = "SDR", device_index: int = 0, **kwargs: Any
    ) -> None:
        message = f"{device_type} device {device_index} not found"
        super().__init__(
            message, sdr_type=device_type, device_index=device_index, **kwargs
        )
...
class DemodulationError(ProcessingError):
    ...
    def __init__(
        self, message: str, modulation_scheme: Optional[str] = None, **kwargs: Any
    ) -> None:
        """
...
class ModelNotTrainedError(ClassificationError):
    """Raised when classification is attempted without trained models.

    Resolution: run 'dronecmd train' to build models from labelled IQ captures,
    or supply a model_path pointing at a directory of trained .pkl files.
    """
...
class SafetyError(ComplianceError):
    ...
    def __init__(
        self, message: str, safety_type: Optional[str] = None, **kwargs: Any
    ) -> None:
        """
...
def create_error_context(
    operation: str, parameters: Dict[str, Any], timestamp: Optional[str] = None
) -> Dict[str, Any]:
    """
...
if __name__ == "__main__":
    ...
    try:
        raise ConfigurationError(
            ...
            config_value=-1000,
            suggestions=[
                "Use a positive sample rate value",
    ...
    test_error = ParsingError(
        "Failed to parse packet", packet_length=0, protocol="mavlink"
    )
