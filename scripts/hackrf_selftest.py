#!/usr/bin/env python3
"""HackRF RX self-test — hardware-in-the-loop validation of DroneCMD.

Drives a **real HackRF** (receive-only) across DroneCMD's capture parameters
and code paths and reports a pass/fail table. It never transmits.
...
Requires the HackRF CLI tools (e.g. PothosSDR) and a connected HackRF. Each
check is isolated: an exception is recorded as a failure and the run continues.
"""
...
def _capture(cfg, n: int) -> np.ndarray:
...
def make_gain_check(label: str, **kw) -> Callable[[], str]:
    def fn() -> str:
...
def check_blind_resolution_rejects_ambient() -> str:
    ...
    # Ambient noise should NOT confidently lock a specific PHY; both correctly
    # returning "no lock" validates the reject path (the loud-on-failure side).
    return (
...
def check_cli_capture_then_analyze() -> str:
    ...
    if ana.returncode != 0:
        ...
        raise _Skip(
            f"capture OK ({size}B); analyze exit {ana.returncode}: "
            f"{blob.strip()[:120]}"
        )
...
def main() -> int:
    ...
    r.run("pipeline/blind_resolution", check_blind_resolution_rejects_ambient)
